...
Using JGR Patch Pack is highly recommended, though not required. AXIS is best played with cargodist (asymmetric equal distribution) enabled for all cargos, and cargo delivery distribution mode set to Balanced, to enable clustering of industries with similar inputs. 

Using compatible vehicle NewGRFs is also highly recommended, but not required in recent versions of OpenTTD. 

## 2.0 Economies

Currently, there are three complete economies; Extreme Classic, Steel City and Tropical Paradise.

Extreme Classic is based on the original XIS economy, and is the successor to the classic "kitchen sink" economies of XIS and older versions of FIRS. It has a little bit of everything and is suitable for any type of map.

Steel City is based on FIRS's "Steeltown" economy, but it adds complete industry chains including a complex petrochemical chain, and new specialised factories for things such as cleaning agents and electrical machines. There is also an improved agricultural chain, but it is not the main focus of this economy.
...
As of version 2.3.0, AXIS uses the FRAX cargo classes.
